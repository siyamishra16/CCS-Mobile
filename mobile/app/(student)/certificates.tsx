...
export default function CertificatesScreen() {
  ...
  // VIEW: Opens in-app browser to preview the PDF
  const handleView = async (url: string) => {
    try {
      await WebBrowser.openBrowserAsync(url, {
        presentationStyle: WebBrowser.WebBrowserPresentationStyle.FULL_SCREEN,
        showTitle: true,
        toolbarColor: BLUE_DARK,
        controlsColor: "#FFFFFF",
      });
    } catch (error) {
      console.error("View error:", error);
      Alert.alert("Error", "Could not open the certificate.");
    }
  };

  // DOWNLOAD: Downloads PDF to cache then opens Android share sheet
  // On Android the share sheet has "Save to Drive", "Save to Downloads" etc.
  const handleDownload = async (url: string, certNo: string, id: number) => {
    try {
      setDownloadingId(id);
      ...
      // This opens the native Android share/save sheet
      // User can pick "Save to Downloads", "Google Drive", etc.
      ...
      <View style={styles.certCard}>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            ...
            disabled={downloadingId === item.id}
            onPress={() =>
              handleDownload(item.file_url, item.certificate_number, item.id)
            }
          >
            {downloadingId === item.id ? (
              <ActivityIndicator size="small" color={BLUE_ACCENT} />
