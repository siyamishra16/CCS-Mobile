    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={() => {setRefreshing(true); fetchExams();}} tintColor={PRIMARY_BLUE} />
          }
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.examCard} activeOpacity={0.9}>
              <View style={styles.cardHeader}>
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>{item.level || 'Intermediate'}</Text>
                </View>
              ...
              <View style={styles.footer}>
                <Text style={styles.examInfo}>{item.total_questions || 0} Questions • Practice</Text>
                <TouchableOpacity 
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              <Text style={styles.empty}>No exams available for this topic.</Text>
            </View>
          }
        />
      )}
